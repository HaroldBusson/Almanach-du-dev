...
if ($_SERVER['REQUEST_METHOD'] == 'POST') { // vérifie si la requête a été envoyé en methode post
    setcookie('name', $_POST['name'], time() + 3600); // Il stocke la valeur du champ username du formulaire pendant 1 heure
...
if (isset($_POST['reset'])) {   // vérifie si le formulaire a été soumis avec un champ nommé reset
    setcookie('name', '', time() - 3600);  // les cookies username en le définissant avec une valeur vide ('') et une date d'expiration dans le passé
